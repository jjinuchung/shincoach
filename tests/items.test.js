...
import { COIN, puzzleCoins, streakCoins, GEAR, DYE, ITEMS, itemById, lootBox, canBuy } from '../js/items.js';
import {
...
test('카탈로그: id가 겹치지 않고 가격은 양수, 장식은 head/face', () => {
  ...
  for (const it of ITEMS) {
    ...
    assert.ok(it.kind === 'gear' || it.kind === 'dye');
  }
...
test('프로필: 장식 장착·교체·벗기 (가방 개수 보존)', () => {
  assert.deepEqual(getLook(25), { gear: null, dye: null });
...
test('백업 병합: 코인·가방·꾸밈은 최근 저장 쪽, 누적치는 큰 값', () => {
  const cur = { id: 'me', xp: 500, coins: 20, coinsEarned: 300, items: { cap: 1 }, mons: { 25: { gear: 'cap' } }, caught: { 25: 2 }, updatedAt: 200 };
  const old = { id: 'me', xp: 400, coins: 90, coinsEarned: 250, items: { crown: 1 }, mons: { 25: { gear: 'crown' } }, caught: { 25: 1, 1: 1 }, updatedAt: 100 };
  ...
  assert.deepEqual(m.mons, { 25: { gear: 'cap' } });
  ...
  assert.equal(m3.coins, 0); assert.deepEqual(m3.items, {}); assert.deepEqual(m3.mons, {});
});

...
| 2026-09-12 | 섀도잉 = 매 재생 뒤 따라 말하기, 그 다음 반복 여부 판단 | 반복 ∞ 기본값에서 반복이 먼저 잡히면 섀도잉이 영영 안 걸림 → 순서 변경 | 반복 끝난 뒤 한 번만 섀도잉 |
...
## 학습 기록 구조 (DB v2)
- 문장 키 = `itemId|round(start*10)` (문장 합치기 설정이 바뀌어도 대체로 유지)
- 메모리 누적 → 5초마다/닫을 때/화면 숨김 때 IndexedDB flush (track.js)
- "완료(done)" = 영어 공개 상태로 끝까지 들음 → 진행률·오늘 목표 기준. "⭐정복" = 발음 점수 80%↑
- 세션은 30초 미만이면 대시보드에서 숨김. 어려운 문장은 말하기 결과 위주(∞ 반복 기본이라 재생 횟수는 약한 신호)

"""⚠️ 이 도구는 실제로 PC를 마비시킨 적이 있다 — 쓰기 전에 아래를 읽을 것 (2026-09-13 사고).

쿵푸팬더4(큐 1,503개)에 돌렸더니 **아버님 PC가 몇 시간 먹통이 되고 스스로 재부팅**됐다.
원인은 CPU가 아니라 **프로세스 생성**이다: 큐마다 ffmpeg를 새로 띄우므로 수천 번이 되고,
Idle 우선순위는 CPU 몫만 낮출 뿐 이걸 막지 못한다. 1,100번째에서 `OSError WinError 299`로 죽었다.
이 PC는 TRADEMY Mobile API가 상시 돌고 있어 더 위험하다.

다시 쓸 일이 있으면 **반드시**:
  · ffmpeg를 큐마다 부르지 말고 `select` 필터로 **한 번에** 뽑을 것
  · 몇 시간짜리면 시작 전에 아버님께 예상 시간·부하를 알리고 승인받을 것
  · 긴 영화(큐 1,000개 이상)에는 애초에 쓰지 말 것 — 한글 자막 파일을 구하는 편이 낫다

...
앱을 켜면 **과목 고르기 카드**(🎤 English / 🔢 Math)가 먼저 나온다. 도감·코인·레벨·학습 기록은 과목이 함께 쓰고, 학습 화면만 과목별로 갈라진다. (수학은 준비 중)
...
- 듣기 먼저 모드: 영어 자막 숨기고 N번 들은 뒤 공개
- 단어 패널: 문장의 모를 만한 단어·표현 한글 뜻 (내장 사전 `vocab/`)
- 말하기 확인: 마이크로 따라 말해야 다음 문장 (음성 인식 일치율 → 폴백: 말소리 길이), 3번 미달 시 통과
- ⚙ 설정은 부모 비밀번호로 보호, 기기 진단(마이크·음성 인식 테스트) 포함
- 📊 학습 기록(부모용, 비밀번호): 주간 요약, 콘텐츠 진행률, 어려워한 문장, 복습 단어장, 세션, JSON 백업/복원. 아이 화면엔 ⭐정복·오늘의 목표만
- 🧬 포켓몬 레벨업·진화: 🔷 수학스톤·🔶 영어스톤으로 키우고(Lv1~12), 정해진 레벨이 되면 진화(꼬부기 → 어니부기 → 거북왕). 진화하면 과목이 바뀌기도 해서 끝까지 가려면 두 과목을 다 해야 한다
